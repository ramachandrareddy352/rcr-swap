...
contract PoolLibrary {
    ...
    // here we send the liquidity(x*y) in range
    // two stable can swap in long range
    // more fee swap in more range
    // more tick more in range
    // tick can be zero so always add tick to some x value
    // final = find how much +amountIn max, -amountOut max based on feee & tick
    // range = (+amountIn * -amountOut)
    ...
    ) public pure returns (uint256 low, uint256 high) {
        require(_reserveIn > 0 && _reserveOut > 0, "Pool Library: Insufficient liquidity");
        uint256 m_range = (_fee + _tick) * _currentLiquidity;

        if(m_range >= _currentLiquidity) {
            (low, high) = (1, _currentLiquidity + m_range);
        } else {
            (low, high) = (_currentLiquidity - m_range, _currentLiquidity + m_range);
        }
    }
